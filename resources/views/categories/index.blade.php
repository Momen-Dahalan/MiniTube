                    ...
                    <ul class="list-group list-group-flush">
                        @foreach ($categories as $cat)
                            <li class="list-group-item px-0 py-1 border-0">
                               ...
                               {{ request('category_id') == $cat->id ? 'bg-primary text-white' : 'bg-light text-dark hover-shadow' }}">
                                    {{ $cat->name }}
                                </a>
                            </li>
                        @endforeach
                                        ...
                                        <div class="card h-100 border-0 shadow-sm hover-shadow transition">
                                            <a href="{{ route('videos.show', $video) }}">
                                                <img src="{{ asset('storage/thumnail/' . $video->thumbnail) }}"
                                                    class="card-img-top" alt="Thumbnail for {{ $video->title }}"
                                                    style="height: 200px; object-fit: cover;">
                                            </a>
                                            <div class="card-body">
                                                <h5 class="card-title mb-1">{{ $video->title }}</h5>
                                                <p class="card-text text-muted small mb-0">{{ __('messages.channel') }}:
